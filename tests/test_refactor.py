...
def test_settings():
    ...
    try:
        ...
        for dir_name, dir_path in [
            ...
            if dir_path.exists():
                print(f"  ✅ {dir_name}: {dir_path} (已存在)")
            else:
                print(f"  ⚠️  {dir_name}: {dir_path} (不存在)")
...
def test_command_line():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"  ❌ 命令行帮助测试失败: {result.stderr}")
            return False
        ...
        if result.returncode == 0:
            ...
            print(f"  ⚠️  info 命令测试失败: {result.stderr}")
...
def test_build_script():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"  ❌ 打包脚本帮助测试失败: {result.stderr}")
            return False
